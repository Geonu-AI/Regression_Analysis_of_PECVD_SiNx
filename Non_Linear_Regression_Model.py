...
W3_sub = tf.Variable(np.random.randn(), name = "W3_sub")
W3_sup = tf.Variable(np.random.randn(), name = "W3_sup")
...
# Hypothesis
y_pred = X1*W1 + X2*W2 + W3_sub*tf.exp(tf.multiply(W3_sup,X3)) + X4*W4 + X5*W5 + b
...
with tf.compat.v1.Session() as sess:
    ...
    for epoch in range(training_epochs):
        ...
        if (epoch + 1) % 50 == 0:
            ...
            print("Epoch", (epoch + 1), ": cost =", c, "W1 =", sess.run(W1), "W2 =", sess.run(W2),"W3_sub =", sess.run(W3_sub),"W3_sup =", sess.run(W3_sup),"W4 =", sess.run(W4),"W5 =", sess.run(W5),"b =", sess.run(b))

            # Storing necessary values to be used outside the Session
    training_cost = sess.run(cost, feed_dict={X1:_x1, X2:_x2,X3:_x3,X4:_x4,X5:_x5, Y:_y})
    weight1 = sess.run(W1)
    weight2 = sess.run(W2)
    weight3_sub = sess.run(W3_sub)
    weight3_sup = sess.run(W3_sup)
    weight4 = sess.run(W4)
    weight5 = sess.run(W5)
    bias = sess.run(b)

#Normalized Weights
print(weight1, weight2, weight3_sub, weight3_sup, weight4, weight5, bias)
